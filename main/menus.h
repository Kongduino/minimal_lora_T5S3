...
void handleModeP2P() {
  ...
  memset(modeButtons[0].value, 0, 10);
  strcpy(myButtons[1].value, "p2p");
  handleMain();
}
...
void handleMode() {
  ...
  displayStatus(txt);
  Serial.println(txt);
  currButtons = modeButtons;
  btnCount = sizeof(modeButtons) / sizeof(kButton);
